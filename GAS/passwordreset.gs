...
function createPersonalAttendanceSheet(memberName) {
  try {
    ...
    // 現在の年のシートを取得
    var sourceSheet = originalSpreadsheet.getSheetByName(
    ...
    console.log("コピー元シート:", currentYear, "→ データ");
...
function aggregateData2025() {
  try {
    ...
    SpreadsheetApp.getUi().alert(
      "完了",
      `${validMembers.length}人のデータを更新しました。`,
      SpreadsheetApp.getUi().ButtonSet.OK
    );
  } catch (error) {
