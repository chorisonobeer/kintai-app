...
Utils.createResponse = function (obj) {
  try {
    ...
    const output = ContentService.createTextOutput(jsonString).setMimeType(
      ContentService.MimeType.JSON
    );

    // 注意: GASのContentServiceはsetHeadersメソッドをサポートしていません
    // CORSヘッダーは自動的に処理されます

    return output;
  } catch (jsonErr) {
    // JSON変換エラーのフォールバック
    const fallbackOutput = ContentService.createTextOutput(
      JSON.stringify({
        ok: false,
        version: VERSION,
...
Utils.computeHash = function (password, salt) {
  ...
  for (let i = 0; i < rawHash.length; i++) {
    const hashByte = (rawHash[i] < 0 ? rawHash[i] + 256 : rawHash[i]).toString(
      16
    );
...
Utils.generateToken = function (userId) {
  ...
  for (let i = 0; i < rawToken.length; i++) {
    const tokenByte = (
      rawToken[i] < 0 ? rawToken[i] + 256 : rawToken[i]
    ).toString(16);
