...
Auth.handleLogin = function (payload, debug, diagInfo) {
  ...
  try {
    ...
    try {
      Auth.storeToken(token, userData.userId, userData.userName);
 ...
 */
Auth.storeToken = function (token, userId, userName) {
  try {
    const tokenInfo = {
      userId: userId,
      userName: userName,
