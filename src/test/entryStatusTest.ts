...
export function testBasicInputLogic(): void {
  ...
  const testCases: Array<{ data: KintaiData; expected: boolean; description: string }> = [
    {
      data: {
        date: '2025-01-01',
        startTime: '09:00',
  ...
  testCases.forEach(({ data, expected, description }) => {
    ...
    console.log(`${status} ${description}: ${result ? '入力済み' : '未入力'} (期待値: ${expected ? '入力済み' : '未入力'})`);
  });
}
...
export async function testEntryStatusManager(): Promise<void> {
  ...
  console.log(`2025-01-01: ${result1.hasEntry ? '入力済み' : '未入力'} (source: ${result1.source})`);
  console.log(`2025-01-02: ${result2.hasEntry ? '入力済み' : '未入力'} (source: ${result2.source})`);
  console.log(`2025-01-03: ${result3.hasEntry ? '入力済み' : '未入力'} (source: ${result3.source})`);
...
export function testLogicComparison(): void {
  ...
  const legacyLogic = (data: KintaiData): boolean => {
    ...
    const hasBreakTime = data.breakTime && data.breakTime.trim() !== '' && data.breakTime !== '0:00';
  ...
  comparison.forEach((result) => {
    ...
    console.log(`${status} ${result.date}: 新=${result.newLogic ? '入力済み' : '未入力'}, 旧=${result.oldLogic ? '入力済み' : '未入力'}`);
  });
}
...
): Array<{ date: string; newLogic: boolean; oldLogic: boolean; match: boolean }> {
